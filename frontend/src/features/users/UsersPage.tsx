...
export function UsersPage() {
  ...
  const headers = [
    ...
    ...(showMatriculation
      ...
      : []),
    ...(showAccounts ? [{ key: 'account', header: 'Account' }] : []),
  ];
  ...
  return (
    <Stack gap={7}>
      <PageHeader
        ...
        description="Manage people and their optional login accounts."
        actions={
          mayCreate ? (
            <Button renderIcon={Add} onClick={() => navigate('/settings/users/new')}>
              Add person
            </Button>
          ) : undefined
        }
      ...
      {peopleQuery.data && (
        <DataTable rows={rows} headers={headers} isSortable>
          {({ rows: tableRows, headers: tableHeaders, getHeaderProps, getRowProps, getTableProps }) => (
            <TableContainer>
              <TableToolbar aria-label="People table toolbar">
                <TableToolbarContent>
                  <Search
                    id="people-search"
              ...
              {rows.length === 0 && (
                <div className="empty-state">
                  ...
                  <p>{search ? 'Try a different search.' : 'No people have been added yet.'}</p>
                </div>
              )}
              <Pagination
