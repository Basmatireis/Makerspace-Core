...
import type { PermissionId } from '../../api/generated/models';
import { createRole } from '../../api/generated/roles/roles';
import { PageHeader } from '../../app/PageHeader';
import { ErrorState, InlineLoadingState } from '../../app/PageState';
import { useCurrentUser } from '../auth/auth';
import { PermissionChecklist } from './PermissionChecklist';
import { permissionListOptions, roleKeys } from './queries';
        ...
        <Tile className="form-tile">
          <Form onSubmit={onSubmit}>
            <Stack gap={7}>
              ...
              <PermissionChecklist permissions={permissionQuery.data.items} selected={selectedPermissions} allowed={currentUser.permissions} onChange={setSelectedPermissions} />
              <div className="form-actions"><Button kind="secondary" type="button" onClick={() => navigate('/settings/roles')}>Cancel</Button><Button type="submit" disabled={createMutation.isPending}>{createMutation.isPending ? 'Creating…' : 'Create role'}</Button></div>
            </Stack>
          </Form>
        </Tile>
      )}
