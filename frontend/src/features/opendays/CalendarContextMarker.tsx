...
export function CalendarContextMarker({ entry, showBreakLabel = true, mode = 'full' }: Props) {
  ...
  const description = academicBreak
    ...
    : `${category}: ${entry.name}`;
  const visibleLabel = mode === 'icon' || (academicBreak && !showBreakLabel) ? '' : entry.name;

  return (
    <span
      className={`calendar-marker calendar-marker--${entry.category} calendar-marker--${mode}${academicBreak && !showBreakLabel ? ' calendar-marker--continuation' : ''}`}
