...
The API emits JSON records to standard output. Request records contain the timestamp, level, message, request ID, HTTP method, generated OpenAPI operation name or normalized route, client IP, bounded User-Agent, status, response-byte count, and duration. A successfully session-authenticated request also contains the opaque Account UUID as `user_id` and the session's `auth_method` (`password`, `pin`, or `oidc`). Anonymous, rejected, SCIM-only, and managed-device-only requests omit both fields. Process lifecycle records include the configured environment where relevant.

Matched requests use generated operation names, with normalized route templates such as `/api/v1/people/{personId}` as a fallback for requests rejected before the generated handler runs. Unmatched requests use only `route: "unmatched"`; raw paths are never logged because arbitrary path segments can contain identifiers or secrets. The `user_agent` field is limited to 512 UTF-8 bytes. Do not log request or response bodies, query searches, contact details, names, matriculation numbers, passwords/hashes, Authorization headers, cookies, session/reset/CSRF tokens, database URLs, arbitrary request headers, or arbitrary error objects. Expected client/auth failures are concise; internal errors retain safe diagnostic context without being returned to the client.

Successful liveness and readiness probes are logged at `DEBUG`, keeping the five-second Compose readiness check out of normal production `INFO` output. Probe responses at HTTP 400 or above remain at `INFO`. Docker Compose may prefix displayed lines with an instance marker such as `backend-1 |`; that multiplexing prefix is not emitted by the application and cannot be removed through application logging configuration.
...
The backend walks `X-Forwarded-For` from right to left, discards configured trusted proxy hops, and records the first untrusted address. A missing or malformed chain falls back to the TCP peer. This single resolved address is also supplied to password and PIN throttling. Every actual trusted hop in a frontend Nginx, Traefik, Cloudflare Tunnel, Pangolin, or multi-proxy deployment must append the address that connected to it, and its network must be configured explicitly. Never add a broad Docker or client-accessible network merely because it also contains a proxy: any peer in a trusted CIDR is authorized to supply forwarding information. When no trusted proxies are configured, all forwarding headers are ignored. `Forwarded`, `X-Real-IP`, and `CF-Connecting-IP` remain ignored.
