...
A period follows `draft ↔ staffing ↔ published → archived`. Managers may return staffing periods to draft or published periods to staffing; assignments and schedule data are retained. Returning to draft hides the period from non-management readers and closes signup, while returning to staffing removes it from public JSON and ICS without closing internal staffing. There is no direct published-to-draft transition and archived periods remain final. Period name and date bounds can change only in draft. Schedule operations are allowed in draft, staffing, and published and are saved as one atomic delta guarded by the period version and affected Open Day versions. Removing a draft slot hard-deletes it; after staffing begins, removal records a cancellation and retains assignments and audit history. Assignment changes never bump the schedule version and are closed on cancelled Open Days and archived periods.
