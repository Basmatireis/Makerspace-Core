...
GitHub may create GHCR packages as private depending on the repository/account settings. Set each backend/frontend package's visibility and repository access in GitHub Packages as appropriate. No additional registry secret is required: the release job alone receives `packages: write`, and the GitHub Release job alone receives `contents: write`.
