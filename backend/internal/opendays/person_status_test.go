...
func openDaysPrincipal(t *testing.T, permissions ...authorization.Permission) authorization.Principal {
	t.Helper()
...
func TestListUpcomingAssignmentsForPersonRequiresBothPersonAndAssignmentRead(t *testing.T) {
	personID := uuid.Must(uuid.NewV7())
	service := &Service{}

	for name, principal := range map[string]authorization.Principal{
		"person read without feature": openDaysPrincipal(t, authorization.PeopleReadAll),
		"feature read without person": openDaysPrincipal(t, authorization.OpenDaysReadAssignments),
	} {
		t.Run(name, func(t *testing.T) {
			_, err := service.ListUpcomingAssignmentsForPerson(t.Context(), principal, personID)
